...
# CREATE Textbook
@textbook_bp.route('/textbooks', methods=['POST'])
def create_textbook():
    ...
    cur.execute("""
        INSERT INTO Textbooks (textbook_title, textbook_author, textbook_version, textbook_isbn, publisher_id)
        VALUES (%s, %s, %s, %s, %s)
        RETURNING id;
...
@textbook_bp.route('/textbooks', methods=['GET'])
def get_textbooks():
    auth_data = authorize_request()
    if isinstance(auth_data, tuple):
        return jsonify(auth_data[0]), auth_data[1]
...
@textbook_bp.route('/textbooks/<int:textbook_id>', methods=['GET'])
...
@textbook_bp.route('/textbooks/<int:textbook_id>', methods=['PATCH'])
def update_textbook(textbook_id):
    ...
    query = f"""
        ...
        RETURNING id, textbook_title, textbook_author, textbook_version, textbook_isbn;
    """
